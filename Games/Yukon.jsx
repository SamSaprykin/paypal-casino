...
function createDeck() {
  ...
  for (let s = 0; s < 4; s++) {
    for (let r = 0; r < 13; r++) {
      d.push({ suit: s, rank: r, faceUp: false, id: `yk-${n++}` });
    }
  }
  return d;
}
...
/** Klondike-style tableau: down, alternating color. */
function canGoOnTableauYukon(moving, targetCol) {
...
/** 52 cards in 7 columns: 1+6+7+8+9+10+11 (same common deal as our Russian build). */
const YUKON_COL_COUNTS = [1, 6, 7, 8, 9, 10, 11];

function initialYukonDeal() {
  const deck = shuffle(createDeck());
  const tableau = Array.from({ length: 7 }, () => []);
  let i = 0;
  for (let c = 0; c < 7; c++) {
    const n = YUKON_COL_COUNTS[c];
    for (let r = 0; r < n; r++) {
...
/**
 * Yukon: any face-up group can move as a block; only the lead (contact) card must
 * fit. Foundations A→K; top foundation card may return to the tableau.
 */
function attemptMove(state, from, toType, toIndex) {
  ...
  if (from.type === "foundation") {
    ...
    if (toType !== "tableau") return null;
    const c = pile[pile.length - 1];
    if (!canGoOnTableauYukon(c, state.tableau[toIndex] ?? [])) return null;
    return {
      ...state,
      foundations: state.foundations.map((p, j) =>
        j === from.index ? p.slice(0, -1) : p,
      ),
...
function findHint(g) {
  for (let fi = 0; fi < 4; fi++) {
    if (g.foundations[fi].length === 0) continue;
    const from = { type: "foundation", index: fi };
...
export default function Yukon() {
  ...
  const [g, setG] = useState(initialYukonDeal);
  ...
  const tryMove = (from, toType, toIndex) => {
    setG((state) => {
      ...
      pushUndo({ g: cloneState(state), moves: movesRef.current });
      setMoves((m) => m + 1);
  ...
  const onFoundationClick = (i) => {
    ...
    if (sel?.type === "foundation" && sel.index === i) {
      setSel(null);
      return;
    }
    if (g.foundations[i].length > 0) {
      setSel({ type: "foundation", index: i });
    } else {
      playErr();
    }
  };
          ...
          {g.foundations.map((pile, i) => (
            <button
              ...
              className={`w-20 h-24 rounded-md border border-amber-900/30 bg-amber-950/20 flex flex-col items-center justify-center text-lg shadow-md transition ${
                ...
                isHintFromFoundation(i)
                  ? "ring-2 ring-amber-300 ring-offset-1 ring-offset-[#0a2e22]"
                  : ""
              } ${
                sel?.type === "foundation" && sel.index === i
                  ? "ring-2 border-amber-400/60"
                  : ""
              }`}
