/**
 * Maps a localized Sanity `WebsitePageIntl` document into a clean, typed section model
 * for Astro pages to dispatch on. Replaces the old Contentful-shaped normalizer.
 *
 * Pipeline:
 *   GraphQL → deepPickIntl(locale) → adaptPageSections() → typed `PageSection[]`.
 */
...
export interface WebsitePage {
  ...
  name?: string;
  slug?: string;
  /** ISO timestamps from Sanity (document level). */
  createdAt?: string;
  updatedAt?: string;
...
function adaptSection(
  ...
  switch (typename) {
    ...
    case "HowToIntl": {
      const items = pickArray<Record<string, unknown>>(b.howToItems).map((it) => ({
        iconName: asString(it.iconName),
        title: asString(it.title),
        steps: pickArray<string>(it.steps).filter((s) => typeof s === "string" && s.trim() !== ""),
      }));
      return {
        kind: "howTo",
        id,
    ...
    case "BonusesListIntl":
      return {
        ...
        id,
        title: asString(b.title),
        bonuses: pickArray(b.bonuses).map(adaptBonus).filter(Boolean) as BonusItem[],
      };
