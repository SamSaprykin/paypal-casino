...
import type { BonusItem, PageSeo } from "../cms/sections";
import {
  getAllCasinoDocumentsWithBodies,
  getCasinoDocumentBySlug,
...
export interface CasinoPage {
  ...
  updatedAt?: string;
  seo?: PageSeo;
  card: import("../cms/cards").CasinoCardData;
...
function adaptFaq(
  ...
  const items = Array.isArray(f.faqItems)
    ? (f.faqItems as Record<string, unknown>[]).flatMap((item) => {
...
function adaptPlayerReview(raw: unknown): CasinoPlayerReview | undefined {
  ...
  if (!description || !personName || !date || rating === undefined)
    return undefined;
  return {
    description,
    personName,
...
export function adaptCasinoPage(
  ...
  const mdxPath = asString(raw.mdxPath);
  ...
  return {
    ...
    userRecommendationsTotalNumber: asNumber(
      raw.userRecommendationsTotalNumber,
    ),
...
export async function getAllCasinos(): Promise<Record<string, unknown>[]> {
  return getAllCasinoDocumentsWithBodies();
}
...
export async function getCasinoReviewBodyMap(): Promise<Map<string, boolean>> {
  if (!casinoReviewBodyMapCache) {
    casinoReviewBodyMapCache = (async () => {
      const rows = await getAllCasinoDocumentsWithBodies();
...
export async function getCasinoBySlug(
  ...
  const [raw, reviewBodyMap, casinoMetaBySlug] = await Promise.all([
    getCasinoDocumentBySlug(normalized, locale),
...
export async function getRootCasinoDetailStaticPaths(): Promise<
