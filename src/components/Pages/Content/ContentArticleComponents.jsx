...
export function InfoArticle({ children }) {
  return (
    <div className="info-card my-8 flex flex-col gap-4 rounded-xl border-l-4 border-blue-400 bg-blue-50/60 px-4 py-4 shadow md:flex-row md:px-8 md:py-6">
      <Info className="h-7 w-7 shrink-0 text-blue-500" aria-hidden />
      <div className="min-w-0 flex-1 text-base leading-relaxed text-blue-900">
        {children}
      </div>
    </div>
  );
...
export function CriteriaItem({ icon = "Shield", title, description }) {
  ...
  return (
    <div className="criteria-item flex gap-3 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
      <Icon className="mt-0.5 h-6 w-6 shrink-0 text-blue-600" aria-hidden />
...
export function EditorNote({ children }) {
  return (
    <aside className="editor-note my-8 flex gap-4 rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-5 italic shadow-sm md:px-6">
      ...
      <div className="min-w-0 text-neutral-800 not-italic [&_p]:mb-2 [&_p]:last:mb-0 [&_p]:leading-relaxed">
        {children}
      </div>
    </aside>
  );
...
export function StepCard({ icon = "UserPlus", stepNum, title, description }) {
  ...
  return (
    <div className="step-card flex flex-col rounded-xl border border-neutral-200 bg-white p-5 text-center shadow-sm">
